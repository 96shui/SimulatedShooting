...
- 根据模型连通组件重新把后手锚点定位到手枪握把中心、前手锚点定位到护木下缘；将独立弹匣顶点组向弹匣井内上移 10 mm，消除悬空间隙。
...
- 2026-07-23 握点与弹匣贴合微调已增加坐标断言；打开的 Unity Editor 已成功重新导入模型、Prefab 和场景，日志未发现 C# 编译错误。Editor 仍持有项目锁，完整 PlayMode 回归需在关闭 Editor 后补跑。
