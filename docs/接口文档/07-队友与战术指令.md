...
| 阶段 | 队友能力 |
|---|---|
| P3 | 3 人纵队：玩家最前、二号队友在后方朝前、三号队友最后朝后；队友位置和状态可用于 HUD/小地图 |
| Later | 换弹掩护、手榴弹、火力支援、前进、停止、拐角拉回玩家、复杂战术动作 |

P1/P2 不实现队友 AI。P3 只要求队形跟随和状态数据可测试；环形菜单和战术指令默认不进入 P3 完成范围。
...
- 玩家换弹时二号队友自动进入掩护换弹状态属于 Later，除非单独提升阶段范围。
- 手榴弹指令必须有有效敌情区域或目标位置。
- 火力支援指令需要敌人或敌情预估方向。
- 停止指令使队友停止前进并保持当前位置。
- 指令失败必须返回原因，UI 不应假装执行成功。
- P3 的 `GetSquadStatus` 必须能返回 3 人纵队位置和状态；`GetAvailableCommands` 可以返回空列表或仅返回 P3 支持命令。
...
P3可用指令为空；Issue/OnReloadStarted战术命令返回InvalidState。状态经ICombatStateService.GetSquadStatus查询，SquadChanged携带SessionId/Revision/Squad。
