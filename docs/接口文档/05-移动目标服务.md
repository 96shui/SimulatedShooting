...
扳机按下/保持/释放和射击调度由 `06-武器与弹药服务.md` 的连射服务负责。移动靶服务只接收已经经过武器、弹药和开火许可校验的逐发结果，不从帧率、Trigger 模拟值或场景枪口对象自行生成射击。`CompleteFireSequence` 用于接收 task007 的序列结束记录；未结束的序列在路线完成时按 `TrainingCompleted` 收口。`FirePhase` 在 task007 接入前由逐发记录推导为 `Idle / InitialTwoShots / ContinuousFire / Stopped`。

`MovingTargetResultDto` 在命中数、弹药消耗之外提供 `TotalShotsFired`、`HitRate01` 和 `ElapsedSeconds`，供结算页直接显示，避免 UI 反推。

## 事件

- `MovingTargetStateChangedEvent`：Tick、开始、计分后发布完整会话快照。
- `MovingTargetShotRecordedEvent`：合法逐发记录成功后发布。
- `MovingTargetCountdownElapsedEvent`：倒计时结束进入运动，展示服务据此结束 P2 倒计时门禁。
- `MovingTargetSessionCompletedEvent`：抵达右端后发布结算，展示服务进入 `SessionResults`。
- `MovingTargetFireSequenceCompletedEvent`：射击序列结束。
