...
| 阶段 | 城镇范围 |
|---|---|
| P3 | 1 个小型城镇场景：一条街、一栋 3 层楼、3-5 个可搜索房间、简单敌人、3 人纵队、简单血量、胜负结算 |
| Later | 第 2 张地图、多栋建筑、复杂观察/伸枪/穿墙校验、高级敌人 AI、高级队友指令 |

P3 敌人 AI 只要求出现、发现玩家、射击和死亡；复杂搜索战术交互可以作为 Later 子能力逐步补充。
...
public readonly struct UrbanSessionDto
{
    ...
    public string CurrentFloorId { get; init; }
    public IReadOnlyList<FloorDto> Floors { get; init; }
...
public readonly struct UrbanResultDto
{
    ...
    public IReadOnlyList<MiniMapDto> FloorMaps { get; init; }
}
```
...
- 城镇地图数量为 2 张。
- P3 可先交付 1 个小型城镇场景，第 2 张地图属于后续扩展。
- 街道敌人 1 到 2 名。
- 建筑敌人 3 到 6 名。
...
- 楼房搜索完毕且消灭所有敌人为胜利。
- 玩家死亡为失败。
- P3 敌人生成、房间搜索和胜负结算必须支持固定随机种子或测试指定数据，保证 PlayMode Test 可复现。
