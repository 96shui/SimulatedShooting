...
- 每个 `docs/BDD/screens/*.feature.md` 文件对应 draw.io 的一个页面。
- 场景使用中文 BDD 写法：`假如 / 当 / 那么 / 而且 / 但是`。
- “玩家”默认指佩戴 VR 头显并使用左右手柄或手部射线交互的训练者。
- “选择”“点击”“确认”在 Unity 中默认通过 XR Ray Interactor 或等效 UI 输入模块触发；无 VR 设备时必须可由输入替身或 XR Device Simulator 触发。
- UI 文本、按钮名、状态名应尽量与 draw.io 页面保持一致，开发时如需改名，需要同步更新 BDD。

## 阶段标签

当前 BDD 覆盖完整目标，但开发按阶段递进。每个实现任务必须先确认对应阶段：

| 标签 | 范围 | 完成口径 |
|---|---|---|
| P1 | 100m 射校闭环、基础输入、基础武器、100m HUD、弹着分析、射校结算 | 已完成历史交付；P2 同步将其入场与轮次体验改为卧姿固定射击 |
| P2 | P1/P2 共用卧姿固定射击体验 + 移动靶闭环 | 两个模式均无人工移动，正前方 UI/步枪与拾枪/轮次显隐闭环可在无 VR/VR 验证 |
...
|---|---|---|
| 产品补充行为 | [00-P1P2卧姿固定射击交互.feature.md](screens/00-P1P2卧姿固定射击交互.feature.md) | P1/P2 卧姿、固定点、无移动、正前方 UI/步枪、拾枪与轮次显隐 |
...
| 09 移动靶 HUD | [09-移动靶HUD.feature.md](screens/09-移动靶HUD.feature.md) | 移动靶 HUD、两发起射/长按连射、命中统计 |
...
- P1/P2 的卧姿固定点、人工移动禁用和大型交互 UI 显隐必须通过共享契约暴露，UI、场景和玩法不得直接引用彼此的具体实现。
...
- XR 输入可用测试替身模拟：射线命中 UI、按下确认、取消、左右手 Grip/Select、右手模拟扳机、头/手姿态、跟踪有效性、换弹、左右肩切换和姿态切换。
- 同一业务服务不得依赖具体硬件输入；模拟输入和真实 XR 输入必须走同一输入适配接口。
- 对涉及随机的敌人、弹着偏移、地图选择，应注入可控随机种子，保证 BDD 示例可重复执行。

## 功能级测试要求

- 每个从 BDD 拆分出来的实现任务，完成时必须同时提供对应测试。
- 纯业务行为优先使用 EditMode 单元测试验证，例如评级算法、偏差计算、弹药规则、状态机和服务错误码。
- 涉及 Unity 对象、Canvas、场景、HUD、按钮点击、XR 输入替身或训练流程的行为，必须使用 PlayMode 测试覆盖。
- 测试名称或测试说明应引用对应 feature 文件和场景名称，保证从 BDD 到测试可追踪。
- 没有对应测试的功能不得作为完成项交付；如暂时只能手工验收，必须写明原因和补测计划。


## P3 契约补充

[25-P3城镇阶段与检查](screens/25-P3城镇阶段与检查.feature.md)：task010同会话阶段、房间检查、街道残敌及结果隔离。

[24-P3堑壕任务与队形](screens/24-P3堑壕任务与队形.feature.md)：task007种子生成、搜索/击杀/结算与导航反馈。

[23-P3共用战斗核心](screens/23-P3共用战斗核心.feature.md)：task004 武器、感知攻击、重复命中、生命周期与无VR输入验证。

[22-P3战斗契约与独立替身](screens/22-P3战斗契约与独立替身.feature.md)：共享类型、配置、三线Fixture、错误和生命周期；编号22为产品补充行为，不恢复原设置页。
