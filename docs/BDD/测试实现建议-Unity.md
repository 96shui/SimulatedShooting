...
当前项目使用 Unity 2022.3.62f3c1、XR Interaction Toolkit 3.1.2、TextMeshPro 3.0.9、UGUI、Input System 1.14.0、OpenXR 1.14.3 和 Unity Test Framework 1.1.33。BDD 文档不要求团队必须使用某个自动化框架，但建议按以下方式落地。
...
每个从 BDD 或接口文档拆分出来的功能，必须把测试作为完成定义的一部分：

- 功能实现前，先确认对应 BDD 场景和接口契约。
- 功能实现时，同步添加或更新 EditMode/PlayMode 测试。
- 功能提测或交付时，说明测试覆盖了哪个 feature 文件和哪个场景。
- 纯业务逻辑没有 EditMode 单元测试时，不视为完成。
- 涉及 UI、场景、HUD、按钮、输入替身或训练流程但没有 PlayMode 测试时，不视为完成。
- 真实 VR 设备尚不可用时，必须先通过输入替身覆盖自动化测试；设备到位后再补充实机验收记录。
